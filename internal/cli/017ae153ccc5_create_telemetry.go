...
func newV017ae153ccc5CreateTelemetryCmd(flags *rootFlags) *cobra.Command {
	var flagAwswafSessionStorage string
	var flagNextInterval int
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if !cmd.Flags().Changed("awswaf-session-storage") && !flags.dryRun {
				return fmt.Errorf("required flag \"%s\" not set", "awswaf-session-storage")
			}
			if !cmd.Flags().Changed("next-interval") && !flags.dryRun {
				return fmt.Errorf("required flag \"%s\" not set", "next-interval")
			}
			...
			path := "/017ae153ccc5/4aa4380fa03e/telemetry"
	...
	cmd.Flags().StringVar(&flagAwswafSessionStorage, "awswaf-session-storage", "", "")
	cmd.Flags().IntVar(&flagNextInterval, "next-interval", 0, "")
